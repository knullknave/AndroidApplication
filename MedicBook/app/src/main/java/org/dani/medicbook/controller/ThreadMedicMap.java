...
import org.dani.medicbook.MedicMapActivity;
...
public class ThreadMedicMap extends AsyncTask<URL, Integer, Long>
{
    public Centro[] centro;
    MedicMapActivity mma;
    public ArrayList<Marker> listaMarkers;

    public ThreadMedicMap(MedicMapActivity mma)
    {
        this.mma = mma;
    ...
    {
        DataBaseManager2 manager2 = new DataBaseManager2(mma);
        ...
        mma.runOnUiThread(new Runnable()
        {
            @Override
            public void run()
            {
                ...
                {
                    listaMarkers.add(mma.map.addMarker(new MarkerOptions().position(new LatLng(centro[i].getPositionx(), centro[i].getPositiony())).title(centro[i].getName()).icon(BitmapDescriptorFactory.fromBitmap(imagen))));
                    listaMarkers.get(i);
                }
            }
        });
